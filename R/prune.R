...
prune <- function(tre, a, train, test=NULL){
  ...
  while (n.tmnl > 1 ) {
   #internal keeps track of all splits which are not terminal <NA> for score value
    ...
    for(j in 1:l) {
      ...
      branch <- tre[is.element(tre$node,c(internal[j], de(internal[j], tree=tre))),]
      score <- as.numeric(as.vector(branch$score))
      
      #r.value is a penalized mean value score across all internal nodes
      r.value[j] <- dim(branch)[1] - nchar(internal[j]) + score[1]/1000
    } 
    ...
    V<-itrtest(dat = train, z=trt.pred, n0=2)
    ...
    V.test <- itrtest(dat = test, z=trt.pred, n0=2)
